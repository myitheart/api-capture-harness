...
`packages/host/directory-picker-native` 经 koffi——它已是仓库其他 `win32.ts` 代码的工作区依赖——在进程内打开 `IFileOpenDialog`（`FOS_PICKFOLDERS | FOS_FORCEFILESYSTEM | FOS_NOCHANGEDIR`），作为 win32 层。COM 会话运行在 spawn 出的子进程中，模态 `Show` 永不阻塞宿主事件循环。子进程在 `Show` 前立即获取当前有效前台窗口并将其作为对话框 owner；浏览器发起的选择器因此保持在该浏览器上方，并在关闭后把焦点还给浏览器，而前台窗口不存在或无效时会明确降级为无 owner 对话框。子进程在阻塞前上报其原生线程 id，驱动层通过向该线程的窗口反复投递 `WM_CLOSE`（`EnumThreadWindows`）来处理中止请求，关闭等待预算耗尽后强制终止子进程。子进程线程启用宿主接受的最佳线程 DPI 感知（`SetThreadDpiAwarenessContext`，按 per-monitor-v2 → per-monitor → system-aware 级联并检查返回值），严格优于脚本的系统 DPI 上限；DPI 保持为纯外观的 best-effort——不接受其中任何一种的宿主仍得到现代对话框，而不会降级。模块切分让覆盖率在任何主机上都诚实：`win32-dialog-logic.ts`（纯时序）与 `win32-dialog.ts`（driver）可在任何平台使用 fake 进行测试；`win32-dialog-bindings.ts` 对 mock 的 `koffi` COM 世界测试（`dsh-session-persistence-jsonl` 的技法）；POSIX 主机运行真实的 spawn 管道，并验证其因 koffi 加载失败而拒绝；win32 主机运行真实的打开对话框并通过中止将其关闭的冒烟测试。先于本层存在的 PowerShell 链已被删除（见[链删除](../simplification/2026-08-04-drop-windows-powershell-picker-fallback.md)）：该层无回退。
...
- **强制选择器全局置顶。** 否决：应用级全局置顶窗口会遮挡无关应用，也不能建立模态 owner 关系或恢复焦点。向 `Show` 传入有效前台窗口，只让选择器相对发起操作的浏览器保持正确层级。

## 后果

- 每台 Windows 机器都得到带其所支持的最佳 DPI 感知（1703+ 为 per-monitor-v2）的现代对话框，无论是否安装 PowerShell。
- 浏览器发起的对话框由前台浏览器窗口拥有，不再依赖 Windows 激活后台服务进程创建的无 owner 窗口；没有有效前台窗口的环境保留无 owner 回退。
