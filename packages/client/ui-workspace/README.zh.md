...
分组浏览器会在真实 Workspace 之前保留一个固定且默认展开的**现场分析**分组。摘要中带有 `agentPreset: 'api-capture-analysis'` 且不属于真实 Workspace 的 Session 会进入该分组，而不是「未分组」。分组的添加操作会在受管分析目录中创建使用同一 Preset 与 Read Only 权限的空白 Session；该分组不提供 Workspace 重命名、删除、目录、排序或拖拽能力。空白分析 Session 无需把技术 cwd 注册为用户 Workspace，也可以正常对话。
